...
@
Display number of lakes cross-classified by the  strata and multidensity category variables
...
@
Lakes in the New England region are displayed in Figure~\ref{fig:Finite}.  The plot function is used to produce the figure.
\begin{verbatim}
...
@
\caption{Lakes in the New England Region.}
\label{fig:Finite}
\end{figure}
