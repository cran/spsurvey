...
The next step is to read the data file, which includes both survey design variables and analytical variables.  The read.delim function is used to read the tab-delimited file and assign it to a data frame named IN\_streams.  The factor function is used to convert the Strahler\_order variable, which contains numeric values, to a factor. A factor is a data structure in R that is used to encode variables that contain a specified set of categorical values, which are referenced as levels. The nrow function is used to determine the number of rows in the IN\_streams data frame, and the resulting value is assigned to an object named nr. Finally, the initial six lines and the final six lines in the IN\_streams data frame are printed using the head and tail functions, respectively.
...
@
\caption{Indiana Stream Sample Sites.}
\label{fig:INstreams}
\end{figure}
