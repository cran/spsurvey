...
There are five additional vignettes in ``spsurvey`` that are available on our website [here](https://usepa.github.io/spsurvey/) under the "Articles" tab:

1. An Overview of Statistical Surveys for Ecological Applications
...
4. Weight Adjustments in Practice
    * Use the `adjwgt()` and `adjwgtNR()` functions to adjust weights for changing sample sizes and nonresponse, respectively.
5. Analyzing Data
...
The version 5.0.0 update to `spsurvey` implemented many significant changes to existing functions. As a result, some of your old code may not run properly while using version 5.0.0. Though we recommend adapting your code to work with the version 5.0.0 update (and later), you may also install a previous version of `spsurvey`. For information regarding the installation of previous version of R packages, please see the RStudio support page [here](https://support.posit.co/hc/en-us/articles/219949047-Installing-older-versions-of-packages). Additionally, old versions of `spsurvey` are also available for download in the release tags section of our GitHub repository [here](https://github.com/USEPA/spsurvey/releases).
