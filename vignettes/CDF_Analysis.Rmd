...
vignette: >
  %\VignetteIndexEntry{CDF Analysis}
...
The next step is to load the data set, which includes both survey design variables and analytical variables.  The data function is used to load the data set and assign it to a data frame named `FL_lakes`.  The `nrow` function is used to determine the number of rows in the `FL_lakes` data frame, and the resulting value is assigned to an object named `nr`. Finally, the initial six lines and the final six lines in the `FL_lake`s data frame are printed using the `head` and `tail` functions, respectively.
...
![Location of small lake sample sites in Florida color-coded by basin.](CDF_Analysis_plot.png)

## CDF analysis of the dissolved oxygen variable
...
```{r cdf_figure, fig.cap="Florida Basins with Significantly Different CDFs."}
