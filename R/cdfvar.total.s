cdfvar.total <- function(z, wgt, x, y, val, cdfest, stratum.ind,
...
#      wgt1 = the stage one final adjusted weight for each site.
...
# Calculate the variance estimate
      ...
      if(!is.null(popsize)) {
         if(vartype == "Local") {
            ...
            varest <- (popsize^2)*((pcfactor*apply(total2est * matrix(rep(wgt1.u, m), nrow = ncluster), 2, localmean.var, weight.lst) + apply(var2est * matrix(rep(wgt1.u, m), nrow = ncluster), 2, sum)) / tw2)
         } else {
            varest <- (popsize^2)*((pcfactor*ncluster*apply(total2est * matrix(rep(wgt1.u, m), nrow = ncluster), 2, var) + apply(var2est * matrix(rep(wgt1.u, m), nrow = ncluster), 2, sum))/ tw2)
         }
      } else {
         if(vartype == "Local") {
            weight.lst <- localmean.weight(x1.u, y1.u, 1/wgt1.u)
