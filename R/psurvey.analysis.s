psurvey.analysis <- function(sites=NULL, subpop=NULL, design=NULL,
   ...
   stage1size=NULL, popcorrect=FALSE, support=NULL, sizeweight=FALSE,
...
#         siteID is a formula); and (2) a variable named use.sites that contains
#         the value TRUE for all sites is created.  The default is NULL.
...
#         siteID argument, or both (when siteID is a formula); and (2) a single  
#         Type variable named all.sites that contains the value "All Sites" for
#         all sites is created.  The default is NULL.
#      design = a data frame consisting of design variables.  If variable names 
...
#         variable is site IDs.  Subsequent variables are response variables.
#         Missing data (NA) is allowed.  The default is NULL.
#      siteID = site IDs.  This variable can be input directly or as a formula 
...
#         variable can be input directly or as a formula and must be supplied
#         either as this argument or in the design data frame.  The default is
#         NULL.
...
#         stage sample.  This variable can be input directly or as a formula and
#         must be supplied either as this argument or in the design data frame
#         when argument vartype is set to "Local".  The default is NULL.
#      ycoord = y-coordinates for location, which either the y-coordinates
...
#         associated with a site from anextensive resource, which is required  
#         for calculation of finite and continuous population correction  
#         factors.  This variable can be input directly or as a formula.  The  
#         default is NULL.
...
#         formula.  The default is NULL.
#      swgt1 = the stage one size-weight for each site.  This variable can be  
#         input directly or as a formula.  The default is NULL.
...
#   Output:
#      A list of class psurvey.analysis.  Only those sites indicated by the 
...
#      least one of the data.cat and data.cont data frames will exist.
#      Depending upon values of the input variables, other elements in the
#      output may be NULL.  The output list is composed of the following
#      elements:
...
#            correction factor is used during variance estimation, where TRUE =
#            use the population correction factor and FALSE = do not use the
#            factor.
#         swgt.ind = a logical value that indicates whether the sample is a
#            size-weighted sample, where TRUE = a size-weighted sample and
#            FALSE = not a size-weighted sample.
#         vartype = the choice of variance estimator, where "Local" = local mean
#            estimator and "SRS" = SRS estimator.
...
#      psurvey.analysis(sites=mysites, subpop=mysubpop[,1:2], design=mydesign,
...
# Create the output list
   ...
   psurvey.obj <- list(sites=sites, subpop=subpop, design=design,
      data.cat=data.cat, data.cont=data.cont, sigma=sigma,
      var.sigma=var.sigma, N.cluster=N.cluster, popsize=popsize,
      stage1size=stage1size, unitsize=unitsize, stratum.ind=stratum.ind,
      cluster.ind=cluster.ind, pcfactor.ind=pcfactor.ind, swgt.ind=swgt.ind,
      vartype=vartype, conf=conf, pctval=pctval)

# Assign class "psurvey.analysis" to the output list

   class(psurvey.obj) <- "psurvey.analysis"
