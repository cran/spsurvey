dcdfvar.size.prop <- function(g, dg, var.sigma, wgt, x, y, cdfest,
   stratum.ind, stratum.level, cluster.ind, cluster, N.cluster, wgt1, x1, y1,
   popsize, pcfactor.ind, stage1size, support, swgt, swgt1, vartype, warn.ind,
   warn.df, warn.vec) {
...
# Calculate the variance estimate
      ...
      if(vartype == "Local") {
         ...
         varest <- (pcfactor*apply(total2est.g * matrix(rep(wgt1.u, m), nrow = ncluster), 2, localmean.var, weight.lst) + apply(var2est * matrix(rep(wgt1.u, m), nrow = ncluster), 2, sum)) / tw2
      } else {
         varest <- (pcfactor*ncluster*apply(total2est.g * matrix(rep(wgt1.u, m), nrow = ncluster), 2, var) + apply(var2est * matrix(rep(wgt1.u, m), nrow = ncluster), 2, sum))/ tw2
      }
