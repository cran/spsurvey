dcdfvar.size.total <- function(g, dg, var.sigma, wgt, x, y, cdfest,
   stratum.ind, stratum.level, cluster.ind, cluster, N.cluster, wgt1, x1, y1,
   popsize, pcfactor.ind, stage1size, support, swgt, swgt1, unitsize, vartype,
...
#      dg = values of the derivative of the deconvolution function g(.)
...
# Calculate the weighted residuals matrix
         ...
         n <- dim(g[[i]])[1]
         if(!is.null(unitsize))
            rm <- (g[[i]] - matrix(rep(cdfest, n), nrow = n, byrow = TRUE)) * matrix(rep(wgt2.lst[[i]], m), nrow = n)
         else
...
# Calculate the variance estimate
      ...
      if(!is.null(unitsize)) {
         if(vartype == "Local") {
            ...
            varest <- (unitsize^2)*((pcfactor*apply(total2est.g * matrix(rep(wgt1.u, m), nrow = ncluster), 2, localmean.var, weight.lst) + apply(var2est * matrix(rep(wgt1.u, m), nrow = ncluster), 2, sum)) / tw2)
         } else {
            varest <- (unitsize^2)*((pcfactor*ncluster*apply(total2est.g * matrix(rep(wgt1.u, m), nrow = ncluster), 2, var) + apply(var2est * matrix(rep(wgt1.u, m), nrow = ncluster), 2, sum))/ tw2)
         }
      } else {
         if(vartype == "Local") {
            weight.lst <- localmean.weight(x1.u, y1.u, 1/wgt1.u)
            varest <- pcfactor*apply(total2est.g * matrix(rep(wgt1.u, m), nrow = ncluster), 2, localmean.var, weight.lst) + apply(var2est * matrix(rep(wgt1.u, m), nrow = ncluster), 2, sum)
         } else {
            varest <- pcfactor*ncluster*apply(total2est.g * matrix(rep(wgt1.u, m), nrow = ncluster), 2, var) + apply(var2est * matrix(rep(wgt1.u, m), nrow = ncluster), 2, sum)
         }
      }
...
# If variance of the estimated measurement error variance is nonzero, add the correction factor to the variance estimate
      ...
      if(!is.null(var.sigma)) {
         ...
         if(!is.null(unitsize))
            varest <- varest + (unitsize^2)*(var.sigma * (dg.sum^2) / tw2)
         else
