...
#   supplies the set of upper bounds for defining the classes for the  CDFs.
...
# Arguments:
#   bounds = vector of upper bounds of the classes for the CDF.
...
# If sample one has two stages, determine whether there are a sufficient number
# of sites in each stage one sampling unit to allow variance calculation
   ...
   if(cluster.ind_1) {
      ...
      if(any(temp)) {
         ...
         stop(paste("\nA variance estimate cannot be calculated since the following stage one sampling \nunit(s) for sample one contain a single site:\n", temp.str, sep=""))
      }
   }

# If sample two has two stages, determine whether there are a sufficient number
# of sites in each stage two sampling unit to allow variance calculation

   if(cluster.ind_2) {
      temp <- sapply(split(cluster_2, cluster_2), length) == 1
      if(any(temp)) {
         temp.str <- vecprint(names(temp)[temp])
         stop(paste("\nA variance estimate cannot be calculated since the following stage one sampling \nunit(s) for sample two contain a single site:\n", temp.str, sep=""))
      }
   }
