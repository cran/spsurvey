################################################################################
# Function: changevar_total (not exported)
...
#'
#' @param size1 The set of category size estimates for survey one.
#'
#' @param size2 The set of category size estimates for survey two.
#'
...
changevar_total <- function(catvar_levels, catvar1, catvar2, wgt, x, y,
  ...
  if (cluster_ind) {
    ...
    for (k in 1:m) {
      ...
      for (i in 1:ncluster) {
        ...
        im <- cbind(z1, z2) * matrix(rep(wgt2_lst[[i]], 2), nrow = n)
        ...
        if (var_ind[i]) {
          if (vartype == "Local") {
            ...
            if (is.null(weight_lst)) {
              ...
              var2est[i, ] <- as.vector(n * var(im))
            } else {
              temp <- localmean_cov(im, weight_lst)
      ...
      if (vartype == "Local") {
        ...
        if (is.null(weight_lst)) {
          ...
          varest <- (localmean_cov(total2est * matrix(rep(wgt1_u, 2),
            ...
            matrix(rep(wgt1_u, 4), nrow = ncluster), 2, sum), nrow = 2))
          temp <- diag(varest)
          if (any(temp < 0)) {
            warn_ind <- TRUE
            act <- "The simple random sampling covariance estimator for an infinite population was used.\n"
    ...
    for (i in 1:m) {
      ...
      if (vartype == "Local") {
        ...
        if (is.null(weight_lst)) {
          ...
          varest <- localmean_cov(im, weight_lst)
          temp <- diag(varest)
          if (any(temp < 0)) {
            warn_ind <- TRUE
            act <- "The simple random sampling covariance estimator for an infinite population was used.\n"
            if (stratum_ind) {
              warn <- paste0("The local mean covariance estimator produced one or more negative variance estimates for \nstratum \"", stratum_level, "\", the simple random sampling covariance estimator for an \ninfinite population was used to calculate covariance of the change estimate for totals.\n")
              warn_df <- rbind(warn_df, data.frame(
                func = I(fname), subpoptype = warn_vec[1],
                subpop = warn_vec[2], indicator = warn_vec[3],
                stratum = stratum_level, warning = I(warn), action = I(act)
