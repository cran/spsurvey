relrisk.analysis <- function(sites, subpop, design, data.rr, response.var,
   stressor.var, response.levels=rep(list(c("Poor","Good")),
   length(response.var)), stressor.levels=rep(list(c("Poor","Good")),
   length(stressor.var)), popsize=NULL, popcorrect=FALSE, pcfsize=NULL,
...
#     one of the subpopulations of that Type.
...
# the data.rr data frame to assure valid contents
   ...
   temp <- dframe.check(sites, design, subpop, data.rr, NULL, design.names)
   design <- temp$design
   subpop <- temp$subpop
   data.rr <- temp$data.cat
...
# Determine whether the subpopulation is empty
            ...
            if(all(is.na(data.rr[subpop.ind,ivar]))) {
               warn.ind <- TRUE
...
# Determine whether the subpopulation contains a single value
            ...
            if(sum(!is.na(data.rr[subpop.ind,ivar])) == 1) {
               warn.ind <- TRUE
...
# Estimate relative risk for the response variable
            ...
            temp.rr <- data.frame(Response=response.var[ivar],
               ...
               LCB95Pct=temp$Results$ConfLimits[1],
               UCB95Pct=temp$Results$ConfLimits[2],
